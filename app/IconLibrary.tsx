...
import { Check, ChevronDown, Clipboard, Code2, Download, FolderInput, Menu, Moon, PenTool, Search, SlidersHorizontal, Sun, Trash2, Upload, X } from "lucide-react";
...
export function IconLibrary() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(savedIcons)) setCustomIcons(savedIcons.filter((icon) => icon?.id && icon?.name && icon?.svg && categories.includes(icon.category)));
    } catch {}
    try {
    ...
    try {
      ...
      if (overrides && typeof overrides === "object") setCategoryOverrides(Object.fromEntries(Object.entries(overrides).filter(([, category]) => categories.includes(category as IconCategory))) as Record<string, IconCategory>);
    } catch {}
  }, []);
  ...
  const counts = useMemo(() => Object.fromEntries(categories.map((category) => [category, allIcons.filter((icon) => icon.category === category).length])), [allIcons]);
    ...
    <div className="app-shell">
      <header className="site-header">
        <button className="brand" onClick={() => setPage("library")} aria-label="打开图标库首页">
          <span className="brand-mark">D</span><span>Deerma Icons</span><span className="version-badge">V1.0.2</span>
        </button>
        <nav className="header-nav" aria-label="主导航">
          <button className={page === "library" ? "active" : ""} onClick={() => setPage("library")}>图标库</button>
        </nav>
        <div className="header-actions">
          <button className="upload-button" onClick={() => setUploadOpen(true)}><Upload/> 上传图标</button>
      ...
      {page === "guidelines" ? <Guidelines onBack={() => setPage("library")}/> : (
        <div className="workspace">
          <aside className={`sidebar ${mobileFilters ? "open" : ""}`}>
            <div className="sidebar-title"><span>图标分类</span><button onClick={() => setMobileFilters(false)} aria-label="关闭筛选"><X/></button></div>
            <button className={`category-row ${selected === null ? "selected" : ""}`} onClick={() => setSelected(null)}><span>全部图标</span><b>{allIcons.length}</b></button>
            {categories.map((category) => <button key={category} aria-pressed={selected === category} className={`category-row ${selected === category ? "selected" : ""}`} onClick={() => toggleCategory(category)}><span>{category}</span><b>{counts[category]}</b></button>)}
          </aside>
      ...
      {uploadOpen && <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setUploadOpen(false)}>
        <section className="upload-modal" role="dialog" aria-modal="true" aria-labelledby="upload-title">
          ...
          <label className="field-label">存入分类<select value={uploadCategory} onChange={(event) => setUploadCategory(event.target.value as IconCategory)}>{categories.map((category) => <option key={category}>{category}</option>)}</select></label>
...
function IconCard({ icon, size, weight, copied, copy, downloadSvg, downloadPng, downloadOutlined, deleteMode, checked, toggleChecked, menuOpen, toggleMenu, closeMenu }: { icon: IconData; size: number; weight: number; copied: string; copy: (text: string, id: string) => void; downloadSvg: (icon: IconData) => void; downloadPng: (icon: IconData) => void; downloadOutlined: (icon: IconData) => void; deleteMode: boolean; checked: boolean; toggleChecked: (id: string, shiftKey?: boolean) => void; menuOpen: boolean; toggleMenu: () => void; closeMenu: () => void }) {
  ...
  return <article className={`icon-card ${checked ? "checked" : ""}`}>
    ...
    <div className="icon-info"><div><h2>{icon.name}</h2><p>{icon.category}</p></div><div className="icon-info-actions" onPointerDown={(event) => event.stopPropagation()}><button className="outline-button" onClick={() => downloadOutlined(icon)} title="下载自动转曲 SVG" aria-label={`${icon.name}下载自动转曲 SVG`}><PenTool/></button><button className="more-button" aria-label={`${icon.name}操作菜单`} aria-expanded={menuOpen} onClick={toggleMenu}><ChevronDown/></button></div></div>
    {menuOpen && <div className="card-menu" onPointerDown={(event) => event.stopPropagation()}>
      ...
      <button onClick={() => { downloadOutlined(icon); closeMenu(); }}><PenTool/>下载转曲 SVG</button>
    </div>}
